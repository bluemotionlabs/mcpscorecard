...
This package is the executable half of the **MCP Server Security Policy**: every check here verifies a numbered section of the policy, and every score the scanner publishes is computed by the code in this repository — weights, grade bands, and pattern lists included. Nothing that decides a score is private. *A score you can't audit is a score you shouldn't trust.*
...
| `capabilities.tool-surface` | §2.1–§2.2 | Tool surface inspectable; high-risk capabilities flagged | Remote `tools/list`, or static scan of the published tarball — **never executed** |
...
| `poisoning.patterns` | §5.1–§5.2 | No hidden instructions / cross-tool shadowing in tool descriptions | Pattern analysis (patterns in [`src/checks/poisoning.ts`](src/checks/poisoning.ts)) |
